...
export const metadata = {
  ...
  description:
    "Step 1.6 of the Phia growth agent â Phia's own ad performance graded against the 2026 hook taxonomy.",
};
    ...
    <main className="shell">
      ...
      <section className="hero">
        ...
        <div className="lede">
          ...
          we use on <a href="/signal">/signal</a> for competitors â so we can
          ...
          into a creative blueprint â that's what we'll prompt Higgsfield with
      ...
      <div className="stat-row">
        ...
        <div className="stat">
          ...
          <div className="value mono">
            {data.blendedCPI > 0 ? `$${data.blendedCPI.toFixed(2)}` : "â"}
          </div>
          <div className="sub">Spend Ã· installs</div>
        </div>
      </div>

      {/* ââ Winning Formula ââ */}
      {formula ? (
        <section className="brief" style={{ marginTop: 56 }}>
          <div className="brief-head">
            ...
            <div className="sub">
              Distilled from your top 3 ads Â· run this through Higgsfield next
            </div>
          </div>
              ...
              <div className="formula-meta">
                {formula.dominantFormat === "ugc-video"
                  ...
                  : formula.dominantFormat === "static"
                    ? "Single image Â· highest ecom conversion share"
                    : formula.dominantFormat === "short-video"
                      ? "5â30s vertical Â· IG-native"
                      : "â"}
              </div>
            </div>
            <div className="formula-cell">
              ...
              <div className="formula-meta">3s view Ã· impressions</div>
            </div>
            <div className="formula-cell">
              <div className="formula-label">Avg CPI (top 3)</div>
              <div className="formula-value mono">
                {formula.avgCPI > 0 ? `$${formula.avgCPI.toFixed(2)}` : "â"}
              </div>
        ...
        <div className="signal-table">
          ...
          {data.hookPerformance.map((row) => (
            <div key={row.hook} className="signal-row hookperf-row">
              ...
              <div className="mono">
                {row.avgCPI > 0 ? `$${row.avgCPI.toFixed(2)}` : "â"}
              </div>
            </div>
          ))}
        </div>
          ...
          {data.formatPerformance.map((row) => (
            <div key={row.format} className="mix-row insights-format-row">
              ...
              <div className="mix-stat mono">
                {row.adCount} ads Â· {fmtPct(row.avgHookRate)} hook Â·{" "}
                {row.avgCPI > 0 ? `$${row.avgCPI.toFixed(2)}` : "â"} CPI
              </div>
            </div>
          ))}
        </div>
